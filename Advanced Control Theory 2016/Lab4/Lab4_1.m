...
W1=[1 minreal(-G(2,1)/G(2,2));minreal(-G(1,2)/G(1,1)) 1];      %MP
% W1=[-G(2,2)/G(2,1) 1;1 -G(1,1)/G(1,2)]*((10*wc)/(s+(10*wc)));                         %NMP
% isproper(W1)

G_bar=minreal(G*W1); %SHOULD BE G*W1!!!!!!!!!!! (doesnt really matter)
evalfr(G_bar,0)
figure(1),bode(G_bar);
...
F_bar=minreal(append(f1,f2))      %mp
% F_bar=minreal ([0 f1;f2 0]);        %nmp    % ===> F_bar!
%%
